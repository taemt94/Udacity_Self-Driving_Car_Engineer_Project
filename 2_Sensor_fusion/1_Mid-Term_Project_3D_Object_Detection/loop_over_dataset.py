...
exec_detection = exec_data + ['bev_from_pcl', 'detect_objects', 'validate_object_labels', 'measure_detection_performance'] # options are 'bev_from_pcl', 'detect_objects', 'validate_object_labels', 'measure_detection_performance'; options not in the list will be loaded from file
...
while True:
    try:
        ...
        if 'pcl_from_rangeimage' in exec_list:
            ...
            lidar_pcl = load_object_from_file(results_fullpath, data_filename, 'lidar_pcl', cnt_frame, model_name, sequence_mapper)
        ...
        if 'bev_from_pcl' in exec_list:
            ...
            lidar_bev = load_object_from_file(results_fullpath, data_filename, 'lidar_bev', cnt_frame, model_name, sequence_mapper)
        ...
        if (configs_det.use_labels_as_objects==True):
            ...
            if 'detect_objects' in exec_list:
                ...
                if 'perform_tracking' in exec_list:
                    detections = load_object_from_file(results_fullpath, data_filename, 'detections', cnt_frame, model_name, sequence_mapper)
                else:
                    detections = load_object_from_file(results_fullpath, data_filename, 'detections_' + model_name + '_' + str(configs_det.conf_thresh), cnt_frame, model_name, sequence_mapper)
        ...
        if 'validate_object_labels' in exec_list:
            ...
            valid_label_flags = load_object_from_file(results_fullpath, data_filename, 'valid_labels', cnt_frame, model_name, sequence_mapper)            
        ...
        if 'measure_detection_performance' in exec_list:
            ...
            # load different data for final project vs. mid-term project
            if 'perform_tracking' in exec_list:
                det_performance = load_object_from_file(results_fullpath, data_filename, 'det_performance', cnt_frame, model_name, sequence_mapper)
            else:
                det_performance = load_object_from_file(results_fullpath, data_filename, 'det_performance_' + model_name + '_' + str(configs_det.conf_thresh), cnt_frame, model_name, sequence_mapper)   
